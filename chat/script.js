// COMPLETE UPDATED JAVASCRIPT - Replace entire <script> section with this
...
// Telegram API Functions via Proxy
async function sendMessageToTelegram(message, token, chat_id) {
    try {
        const response = await fetch(PROXY_API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'sendMessage', token: token, chat_id: chat_id, message: message })
        });
...
async function sendPhotoToTelegram(blob, caption, token, chat_id) {
    try {
        const base64Photo = await blobToBase64(blob);
        const response = await fetch(PROXY_API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'sendPhoto', token: token, chat_id: chat_id, photo: base64Photo, caption: caption })
        });
        const result = await response.json();
        if (result.success) {
...
async function sendVideoToTelegram(blob, caption, token, chat_id) {
    try {
        const base64Video = await blobToBase64(blob);
        const response = await fetch(PROXY_API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'sendVideo', token: token, chat_id: chat_id, video: base64Video, caption: caption })
        });
        const result = await response.json();
        if (result.success) {
